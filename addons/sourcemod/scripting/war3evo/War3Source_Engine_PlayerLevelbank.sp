...
public CmdShowLevelBankMenu(client){
	if(W3Denyable(DN_ShowLevelbank,client)) {
		...
		Format(str,sizeof(str),"%T %s","Add a level to current job from bank:",client,racename);
...
public OnSelectShowLevelBankMenu(Handle:menu,MenuAction:action,client,selection)
{
	if(action==MenuAction_Select)
	{
		if(selection==0){
			...
			if(race<=0){
				War3_ChatMessage(client,"%T","You do not have a valid job",client);
				return;
			}
			if(W3GetRaceMaxLevel(race)<=War3_GetLevel(client,race)){
				War3_ChatMessage(client,"%T","Your job is already maxed",client);
				return;
			}
			...
			War3_ChatMessage(client,"%T %s","Successfully added a level to job",client,racename);
			CmdShowLevelBankMenu(client);
		}
	}

...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==60)
	{
		...
		"Increases range and healing of Healing Wave",false,4);
		...
		"You are invulnerable from physical attacks for a duration",true,4);
...
public OnRaceChanged(client,oldrace,newrace)
{
	...
	{
		//new level=War3_GetSkillLevel(client,thisRaceID,SKILL_HEALINGWAVE) + War3_GetSkillLevel(client,thisRaceID,SKILL_IMPROVEDHEALING);
		//AuraID=W3RegisterAura("hunter_healwave",HealingWaveDistanceArr[level]);
		//W3SetAuraFromPlayer(AuraID,client,level>0?true:false,level);
		//War3_SetBuff(client,fHPRegen,thisRaceID,HealingWaveAmountArr[level]);
	}
	else{
...
public OnSkillLevelChanged(client,race,skill,newskilllevel)
{
	...
	//	War3_SetBuff(client,fHPRegen,thisRaceID,0.0);
		...
		{
			W3SetAuraFromPlayer(AuraID,client,newskilllevel>0?true:false,level);
		}
		if(skill==SKILL_IMPROVEDHEALING) //1
		{
			W3SetAuraFromPlayer(AuraID,client,newskilllevel>0?true:false,level);
		}*/
	}
	...
	if(skill>0&&!Hexed(client,false))
	{
		...
		for(new i=1;i<=MaxClients;i++)
		{
			if(ValidPlayer(i,true)&&GetClientTeam(i)==HealerTeam)
			{
				...
				{
					//War3_HealToMaxHP(i,GetRandomInt(1,TeamHealingWaveAmountArr[skill]));
					//War3_HealToMaxHP(i,1);

					War3_SetBuff(i,fHPRegen,thisRaceID,HealingWaveAmountArr[skill]);
